...
// Returns an empty string if not in a git repo or no main worktree found.
...
// main worktree and all linked worktrees. Each path is absolute.
// Returns nil if workDir is not in a git repository.
...
// Returns an empty string if the path is the main worktree or not found.
...
// Returns (exists, mainPath) where mainPath is the path to switch to if the worktree was deleted.
