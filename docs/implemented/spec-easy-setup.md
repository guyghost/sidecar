...
Hosted at: `https://raw.githubusercontent.com/guyghost/sidecar/main/scripts/setup.sh`
...
6. **Compares versions** - Skip reinstall if already up-to-date
   ...
   if [[ "$LOCAL_VERSION" == "$LATEST_VERSION" ]]; then
     ...
     echo "Will run:"
     echo "  go install -ldflags \"-X main.Version=$LATEST\" github.com/guyghost/sidecar/cmd/sidecar@$LATEST"
     # prompt, then run
   fi
   ```
...
Update: curl -fsSL https://raw.githubusercontent.com/guyghost/sidecar/main/scripts/setup.sh | bash
```
